...
export interface DeviceOptions {
  ...
  onError: (deviceModel: DeviceInfo, err) => void;
...
export class Device {
  ...
  constructor(options: DeviceOptions, logger: Logging) {
    ...
    this.logger.info(`host: ${this.deviceOptions.host}`);
    this.logger.info(`port: ${this.deviceOptions.port}`);
  ...
  _setDevice(id: string, name: string, address: string, port: number) {
    ...
    this.logger.info(`Device identified ${id}, ${name}, ${address}, ${port}`);
  }
  ...
  _handleResponse(msg: any, rinfo: dgram.RemoteInfo) {
    if (rinfo.address !== this.deviceOptions.host) {
      this.logger.info(
        `Received response from unexpected address ${rinfo.address}`,
      );
    ...
    try {
      ...
      const pack = this.encryptionService.decrypt(
        message,
        (this.deviceInfo || {}).key,
      );
      ...
      if (pack.t === 'bindok') {
        ...
        setInterval(
          this._requestDeviceStatus.bind(this, this.deviceInfo),
          this.deviceOptions.updateInterval,
        );
      ...
      if (pack.t === 'res' && this.deviceInfo.bound) {
        if (!pack.val) {
          this.logger.warn('Updating device prop failed - pack.val empty, requesting update');
          this._requestDeviceStatus(this.deviceInfo);
          return;
        }

        pack.opt.forEach((opt: any, i: any) => {
          if (pack.val && pack.val[i]) {
            this.logger.info(`Updating device prop: ${opt} w/ index ${i}`);
            this.deviceInfo.props[opt] = pack.val[i];
          } else {
            this.logger.warn(
              `Updating device prop: ${opt} w/ index ${i} failed - pack.val empty`,
            );
          }
        });
      ...
      this.deviceOptions.onError(this.deviceInfo, 'unknown');
    } catch (err) {
      this.deviceOptions.onError(this.deviceInfo, err);
    }
  }
  ...
  _sendRequest(
    ...
    this.logger.debug(`Send command: ${JSON.stringify(message)}`);
  ...
  setPower(value: number) {
    this._sendCommand([Commands.power.code], [value ? 1 : 0]);
  }
  ...
  setMode(value: number) {
    this._sendCommand([Commands.mode.code], [value]);
  }

  getMode(): number {
    return (
      this.deviceInfo.props[Commands.mode.code] || Commands.mode.value.auto
    );
  }
  ...
  setFanSpeed(value: number) {
    this._sendCommand([Commands.fanSpeed.code], [value]);
  }

  getFanSpeed(): number {
    return (
      this.deviceInfo.props[Commands.fanSpeed.code] ||
      Commands.fanSpeed.value.auto
    );
  }
  ...
  setSwingVert(value: number) {
    this._sendCommand([Commands.swingVert.code], [value]);
  }

  getSwingVert(): number {
    return (
      this.deviceInfo.props[Commands.swingVert.code] ||
